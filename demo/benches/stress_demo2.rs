...
fn tier4_integration_worker_exchange(ctx: &mut StressContext) {
    ...
    let completed = ctx.measure_batch("worker exchange", exchanges, || {
...
#[stress(tier = 5, metadata(tier_name = "saturation_scaling"))]
fn tier5_saturation_scaling_client_fanout(ctx: &mut StressContext) {
    let clients = 32_u64;
    ctx.parameter("client_count", clients);

    let completed = ctx.measure_batch("client fanout", clients, || {
        let mut total = 0_u64;
...
#[stress(tier = 6, metadata(tier_name = "soak_endurance"))]
fn tier6_soak_endurance_error_free_loop(ctx: &mut StressContext) {
    ...
    let completed = ctx.measure_batch("error free loop", iterations_per_batch, || {
